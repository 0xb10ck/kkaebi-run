...
func take_damage(amount: int) -> void:
	...
	hp = max(0, hp - amount)
	hp_changed.emit(hp, max_hp)
	EventBus.player_damaged.emit(amount, &"contact")
	if hp <= 0:
		_dead = true
		died.emit()
...
func _on_exp_pickup_area_entered(area: Area2D) -> void:
	...
	if adjusted < 1:
		adjusted = 1
	_add_exp(adjusted)
	EventBus.xp_collected.emit(adjusted)
...
# §6.3 메타 강화 → 플레이어 스탯 적용. MetaState의 폴백 표를 그대로 사용.
func _apply_meta_bonuses() -> void:
	var hp_mult: float = MetaState.get_upgrade_effect(&"max_hp")
	if hp_mult > 0.0:
		max_hp = int(round(float(BASE_MAX_HP) * (1.0 + hp_mult)))
		hp = max_hp
	var ms_mult: float = MetaState.get_upgrade_effect(&"move_speed")
	if ms_mult > 0.0:
		move_speed_mult *= (1.0 + ms_mult)
	var xp_mult: float = MetaState.get_upgrade_effect(&"xp_gain")
	if xp_mult > 0.0:
		exp_gain_mult *= (1.0 + xp_mult)
